...
The system consists of two main environments communicating via REST API and async events. Both follow **Domain-Driven Design (DDD)** in their Domain layers: ubiquitous language, Aggregates enforcing their own invariants, Value Objects (no primitive obsession — any primitive carrying domain meaning is wrapped, not passed raw), Domain Services, Domain Events, and each vertical-slice module as its own Bounded Context. Established domain archetypes/analysis patterns (e.g., Party, Product, Rule Engine) should be considered per concept but only adopted where they genuinely fit MES's own rules — see `CLAUDE.md`.
